...
fn param_config(param: mldsa.ParameterSet) -> Config {
  case param {
    mldsa.Mldsa44 -> Config(k: 4, l: 4, eta: 2)
    mldsa.Mldsa65 -> Config(k: 6, l: 5, eta: 4)
    mldsa.Mldsa87 -> Config(k: 8, l: 7, eta: 2)
  }
}
...
fn expand_a(rho: BitArray, config: Config) -> List(List(List(Int))) {
  int.range(from: config.k - 1, to: -1, with: [], run: fn(acc, i) {
    let row =
      int.range(from: config.l - 1, to: -1, with: [], run: fn(acc, j) {
        [rej_ntt_poly(rho, j, i), ..acc]
      })
    [row, ..acc]
  })
}
...
fn expand_s(
  ...
  let s1 =
    int.range(from: config.l - 1, to: -1, with: [], run: fn(acc, nonce) {
      [rej_bounded_poly(rho_prime, nonce, config.eta), ..acc]
    })
  let s2 =
    int.range(from: config.k - 1, to: -1, with: [], run: fn(acc, i) {
      [rej_bounded_poly(rho_prime, config.l + i, config.eta), ..acc]
    })
  #(s1, s2)
}
