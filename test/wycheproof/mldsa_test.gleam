...
fn has_non_empty_context(ctx: Option(String)) -> Bool {
  option.map(ctx, fn(value) { value != "" }) |> option.unwrap(False)
}
...
fn run_verify_tests(filename: String, param: mldsa.ParameterSet) -> Nil {
...
pub fn wycheproof_mldsa_44_verify_test() {
  ...
  run_verify_tests("mldsa_44_verify_test.json", mldsa.Mldsa44)
}

pub fn wycheproof_mldsa_65_verify_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_verify_tests("mldsa_65_verify_test.json", mldsa.Mldsa65)
}

pub fn wycheproof_mldsa_87_verify_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_verify_tests("mldsa_87_verify_test.json", mldsa.Mldsa87)
}
...
fn check_verification(
  ...
  case result {
    utils.Valid -> {
      assert mldsa.verify(pub_key, msg_bytes, signature: sig_bytes)
        as { label <> " verification failed: " <> context }
    }
    utils.Invalid -> {
      assert !mldsa.verify(pub_key, msg_bytes, signature: sig_bytes)
        as { label <> " verification succeeded for invalid: " <> context }
    }
    utils.Acceptable -> {
      let _ = mldsa.verify(pub_key, msg_bytes, signature: sig_bytes)
      Nil
...
fn run_seed_sign_test(
  ...
  let context = utils.test_context(tc.tc_id, tc.comment)
  use <- bool.guard(when: has_non_empty_context(tc.ctx), return: Nil)
  use <- bool.guard(when: tc.msg == "", return: Nil)
  use <- bool.guard(when: option.is_none(group.public_key), return: Nil)

  let assert option.Some(pk_hex) = group.public_key
...
fn run_seed_sign_tests(filename: String, param: mldsa.ParameterSet) -> Nil {
...
pub fn wycheproof_mldsa_44_sign_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_seed_sign_tests("mldsa_44_sign_seed_test.json", mldsa.Mldsa44)
}

pub fn wycheproof_mldsa_65_sign_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_seed_sign_tests("mldsa_65_sign_seed_test.json", mldsa.Mldsa65)
}

pub fn wycheproof_mldsa_87_sign_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_seed_sign_tests("mldsa_87_sign_seed_test.json", mldsa.Mldsa87)
}

fn run_keygen_test(filename: String, param: mldsa.ParameterSet) -> Nil {
  let assert Ok(file) = utils.load_test_file(filename, seed_test_file_decoder())
  list.each(file.test_groups, fn(group) {
    let assert Ok(seed) = bit_array.base16_decode(group.private_seed)
    case group.public_key {
      option.None -> {
        assert mldsa_internal.public_key_from_seed(param, seed) == Error(Nil)
      }
      option.Some(pk_hex) -> {
        let assert Ok(expected_pk) = bit_array.base16_decode(pk_hex)
        let assert Ok(derived_pk) =
          mldsa_internal.public_key_from_seed(param, seed)
...
pub fn wycheproof_mldsa_44_keygen_from_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_keygen_test("mldsa_44_sign_seed_test.json", mldsa.Mldsa44)
}

pub fn wycheproof_mldsa_65_keygen_from_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_keygen_test("mldsa_65_sign_seed_test.json", mldsa.Mldsa65)
}

pub fn wycheproof_mldsa_87_keygen_from_seed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_keygen_test("mldsa_87_sign_seed_test.json", mldsa.Mldsa87)
}
...
fn run_noseed_sign_test(
  ...
  case group.public_key, tc.msg, has_non_empty_context(tc.ctx) {
    option.Some(pk_hex), option.Some(msg_hex), False ->
      run_noseed_sign_check(param, tc, pk_hex, msg_hex)
    _, _, _ -> Nil
  }
}

fn run_noseed_sign_check(
  param: mldsa.ParameterSet,
  tc: NoseedTestCase,
  pk_hex: String,
  msg_hex: String,
...
fn run_noseed_sign_tests(filename: String, param: mldsa.ParameterSet) -> Nil {
  let assert Ok(file) =
...
pub fn wycheproof_mldsa_44_sign_noseed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_noseed_sign_tests("mldsa_44_sign_noseed_test.json", mldsa.Mldsa44)
}

pub fn wycheproof_mldsa_65_sign_noseed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_noseed_sign_tests("mldsa_65_sign_noseed_test.json", mldsa.Mldsa65)
}

pub fn wycheproof_mldsa_87_sign_noseed_test() {
  use <- unitest.guard(mldsa.supported())
  use <- unitest.tag("wycheproof")
  run_noseed_sign_tests("mldsa_87_sign_noseed_test.json", mldsa.Mldsa87)
}
