...
pub fn compute_crt_params_invariants_property_test() {
  ...
  let assert Ok(n_val) = bigi.from_bytes(n_bytes, bigi.BigEndian, bigi.Unsigned)
  let assert Ok(d_val) = bigi.from_bytes(d_bytes, bigi.BigEndian, bigi.Unsigned)
  ...
  let assert Ok(dp) = bigi.from_bytes(dp_bytes, bigi.BigEndian, bigi.Unsigned)
  let assert Ok(dq) = bigi.from_bytes(dq_bytes, bigi.BigEndian, bigi.Unsigned)
  let assert Ok(qi) = bigi.from_bytes(qi_bytes, bigi.BigEndian, bigi.Unsigned)
