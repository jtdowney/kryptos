...
pub fn hmac_sha256_rfc4231_test_case_1_test() {
  ...
  let assert Ok(result) = hmac.compute(hash.Sha256, key, data)
...
pub fn hmac_sha512_rfc4231_test_case_1_test() {
  ...
  let assert Ok(result) = hmac.compute(hash.Sha512, key, data)
...
pub fn hmac_sha1_test() {
  ...
  let assert Ok(result) = hmac.compute(hash.Sha1, key, data)
...
pub fn hmac_md5_test() {
  ...
  let assert Ok(result) = hmac.compute(hash.Md5, key, data)
...
pub fn hmac_algorithms_table_test() {
  ...
  |> dict.each(fn(algorithm, expected) {
    let assert Ok(output) = hmac.compute(algorithm, key, data)
...
pub fn incremental_hmac_test() {
  ...
  let assert Ok(expected) = hmac.compute(hash.Sha256, key, data)

  let assert Ok(hmac) = hmac.new(hash.Sha256, key)
  let result =
    hmac
...
pub fn incremental_no_updates_test() {
  let key = <<"key":utf8>>
  let assert Ok(expected) = hmac.compute(hash.Sha256, key, <<>>)
  let assert Ok(hmac) = hmac.new(hash.Sha256, key)
  let result = hmac.final(hmac)
...
pub fn verify_valid_mac_test() {
  ...
  let assert Ok(mac) = hmac.compute(hash.Sha256, key, data)
...
pub fn unsupported_algorithm_compute_test() {
  let key = <<"key":utf8>>
  let data = <<"data":utf8>>
  let result = hmac.compute(hash.Sha3x256, key, data)
  assert result == Error(Nil)
}
